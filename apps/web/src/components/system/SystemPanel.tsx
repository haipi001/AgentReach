...
export function SystemPanel() {
  ...
  const [backupPreview, setBackupPreview] = useState<OwnerBackup | null>(null);
  ...
  async function exportOwner() {
    ...
    setBackupBusy(true); setIdentityError("");
    try {
      const backup = await identityApi.export();
      const url = URL.createObjectURL(new Blob([JSON.stringify(backup)], { type: "application/json" }));
      const link = document.createElement("a");
      link.href = url;
      link.download = `agentreach-${backup.profile.display_name}-${backup.exported_at.slice(0, 10)}.json`;
      link.click();
      URL.revokeObjectURL(url);
    } catch (error) { setIdentityError(error instanceof Error ? error.message : "Owner 备份导出失败"); }
  ...
  async function inspectBackup(event: ChangeEvent<HTMLInputElement>) {
    ...
    event.target.value = "";
    if (!file) return;
    setIdentityError(""); setConfirmRestore(false);
    try {
      const parsed = JSON.parse(await file.text()) as OwnerBackup;
      if (parsed.format !== "agentreach-owner-backup/v1" || !parsed.database_base64 || !parsed.database_sha256 || !parsed.profile?.display_name) throw new Error("这不是有效的 AgentReach Owner 备份");
      setBackupPreview(parsed);
      setRestoreName(`${parsed.profile.display_name}（恢复）`);
    } catch (error) { setBackupPreview(null); setIdentityError(error instanceof Error ? error.message : "无法读取备份文件"); }
  }
  ...
  async function restoreOwner() {
    ...
    setBackupBusy(true); setIdentityError("");
    try {
      const next = await identityApi.restore(backupPreview, restoreName);
      setDemo(next);
      const profile = next.identity_runtime?.profiles.find((item) => item.active);
      if (profile) setPersona({ name: profile.agent_name });
  ...
  function beginProfileEdit(profileId: string, displayName: string, currentAgentName: string) {
    setEditingProfile(profileId); setProfileOwnerName(displayName); setProfileAgentName(currentAgentName);
    setConfirmDeleteProfile(null); setIdentityError("");
  }

  async function renameProfile() {
    if (!editingProfile || identityBusy) return;
    setIdentityBusy(true); setIdentityError("");
    try {
      const runtime = await identityApi.rename(editingProfile, profileOwnerName, profileAgentName);
      const next = await demoApi.get(); setDemo(next);
      const active = runtime.profiles.find((item) => item.active);
      if (active) setPersona({ name: active.agent_name });
      setEditingProfile(null);
    } catch (error) { setIdentityError(error instanceof Error ? error.message : "身份更新失败"); }
    finally { setIdentityBusy(false); }
  }

  async function deleteProfile(profileId: string) {
    if (identityBusy) return;
    if (confirmDeleteProfile !== profileId) { setConfirmDeleteProfile(profileId); setEditingProfile(null); return; }
    setIdentityBusy(true); setIdentityError("");
    try {
      const runtime = await identityApi.delete(profileId);
      setDemo(await demoApi.get()); setConfirmDeleteProfile(null);
      if (!runtime.profiles.length) throw new Error("至少需要保留一个本地身份");
    } catch (error) { setIdentityError(error instanceof Error ? error.message : "身份删除失败"); }
    finally { setIdentityBusy(false); }
  }
  ...
  return <AnimatePresence>{open && <>
    ...
    <motion.aside className="system-panel" aria-label="AgentReach 系统控制面" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 260, damping: 30 }}>
      ...
      <div className="system-panel-body">
        {tab === "owner" && <section><div className="system-panel-heading"><span>LOCAL OWNER SESSION</span><b>{demo?.identity_runtime?.local_only ? "LOCAL ONLY" : "UNAVAILABLE"}</b></div><div className="owner-session"><span>ACTIVE SESSION</span><strong>{demo?.identity_runtime?.session_id ?? "LEGACY SESSION"}</strong><p>每个本地身份拥有独立的任务、Memory、Skill、Connector 和 Inbox 数据库。</p></div><div className="owner-profiles">{demo?.identity_runtime?.profiles.map((profile) => <article key={profile.profile_id} className={profile.active ? "active" : ""}><i/><div><strong>{profile.display_name}</strong><p>{profile.agent_name} / PERSONAL AGENT</p></div><small>{profile.active ? "CURRENT" : profile.profile_id}</small><div className="profile-actions">{!profile.active && <button onClick={() => switchIdentity(profile.profile_id)} disabled={identityBusy}>切换</button>}<button onClick={() => beginProfileEdit(profile.profile_id, profile.display_name, profile.agent_name)} disabled={identityBusy}>编辑</button>{!profile.active && profile.profile_id !== "local-owner" && <button className="danger-control" onClick={() => deleteProfile(profile.profile_id)} disabled={identityBusy}>{confirmDeleteProfile === profile.profile_id ? "确认删除" : "删除"}</button>}</div>{editingProfile === profile.profile_id && <form className="owner-profile-edit" onSubmit={(event) => { event.preventDefault(); renameProfile(); }}><input aria-label="所有者名称" value={profileOwnerName} onChange={(event) => setProfileOwnerName(event.target.value)} maxLength={40}/><input aria-label="AI 名称" value={profileAgentName} onChange={(event) => setProfileAgentName(event.target.value)} maxLength={40}/><button type="button" onClick={() => setEditingProfile(null)}>取消</button><button disabled={identityBusy || !profileOwnerName.trim() || !profileAgentName.trim()}>保存身份</button></form>}</article>)}</div>{(demo?.identity_runtime?.profiles.length ?? 0) < 5 && <form className="owner-create" onSubmit={(event) => { event.preventDefault(); createIdentity(); }}><span>CREATE ISOLATED IDENTITY</span><input value={ownerName} onChange={(event) => setOwnerName(event.target.value)} placeholder="所有者名称" maxLength={40}/><input value={agentName} onChange={(event) => setAgentName(event.target.value)} placeholder="AI 名称" maxLength={40}/><button disabled={identityBusy || !ownerName.trim() || !agentName.trim()}>{identityBusy ? "创建中" : "创建本地身份"}</button></form>}<div className="owner-portability"><header><div><span>PORTABLE OWNER DATA</span><strong>备份与非覆盖恢复</strong></div><b>SHA-256</b></header><p>导出当前 Owner 的任务、记忆、技能和授权。恢复时始终创建新身份，不会覆盖已有数据。</p><div className="owner-backup-actions"><button onClick={exportOwner} disabled={backupBusy}>{backupBusy ? "处理中" : "导出 Owner 备份"}</button><label>选择备份文件<input type="file" accept=".json,application/json" onChange={inspectBackup}/></label></div><small>⚠ 备份保留在本机且带完整性校验，但当前版本未加密，请勿发送给他人。</small>{backupPreview && <div className="owner-backup-preview"><header><span>RESTORE PREVIEW</span><b>{Math.ceil(backupPreview.bytes / 1024)} KB</b></header><strong>{backupPreview.profile.display_name} / {backupPreview.profile.agent_name}</strong><p>{backupPreview.exported_at.slice(0, 19).replace("T", " ")} · {backupPreview.database_sha256.slice(0, 16)}…</p><input value={restoreName} onChange={(event) => { setRestoreName(event.target.value); setConfirmRestore(false); }} maxLength={40} aria-label="恢复后的身份名称"/><div><button onClick={() => { setBackupPreview(null); setConfirmRestore(false); }} disabled={backupBusy}>取消</button><button className={confirmRestore ? "danger" : ""} onClick={restoreOwner} disabled={backupBusy || !restoreName.trim()}>{confirmRestore ? "确认创建恢复身份" : "恢复为新身份"}</button></div></div>}</div>{identityError && <p className="owner-error">{identityError}</p>}</section>}
